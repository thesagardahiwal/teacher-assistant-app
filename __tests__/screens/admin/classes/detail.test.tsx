import EditClass from "@/app/(admin)/classes/[id]";
import { classService } from "@/services";
import { fireEvent, render, waitFor } from "@testing-library/react-native";
import React from "react";
import { Alert } from "react-native";

// Mock Hooks
jest.mock("react-native-safe-area-context", () => ({
    SafeAreaView: ({ children }: any) => <>{children}</>,
    useSafeAreaInsets: () => ({ top: 0, bottom: 0, left: 0, right: 0 }),
}));
jest.mock("@/store/hooks/useTheme", () => ({ useTheme: () => ({ isDark: false }) }));
jest.mock("@/store/hooks/useAuth", () => ({ useAuth: () => ({ user: { role: "ADMIN" } }) }));
jest.mock("@/utils/useInstitutionId", () => ({ useInstitutionId: () => "inst1" }));

jest.mock("@/store/hooks/useClasses", () => ({ useClasses: () => ({ fetchClasses: jest.fn() }) }));
jest.mock("@/store/hooks/useCourses", () => ({ useCourses: () => ({ data: [{ $id: "c1", name: "C1", code: "CODE" }], fetchCourses: jest.fn() }) }));
jest.mock("@/store/hooks/useAcademicYears", () => ({ useAcademicYears: () => ({ data: [{ $id: "ay1", label: "2024" }], fetchAcademicYears: jest.fn() }) }));

jest.mock("expo-router", () => ({
    useRouter: () => ({ back: jest.fn() }),
    useLocalSearchParams: () => ({ id: "cl1" }),
}));

jest.mock("@/services", () => ({
    classService: {
        get: jest.fn(),
        update: jest.fn(),
        delete: jest.fn(),
    }
}));

// Mock components
jest.mock("@/components/admin/ui/FormInput", () => {
    const { TextInput } = require('react-native');
    return {
        FormInput: ({ label, value, onChangeText }: any) => (
            <TextInput testID={`input-${label}`} value={value} onChangeText={onChangeText} />
        )
    };
});
jest.mock("@/components/admin/ui/FormSelect", () => {
    const { TouchableOpacity, Text } = require('react-native');
    return {
        FormSelect: ({ label, onChange, options }: any) => (
            <TouchableOpacity testID={`select-${label}`} onPress={() => options.length > 0 && onChange(options[0].value)}>
                <Text>{label}</Text>
            </TouchableOpacity>
        )
    };
});
jest.mock("@/components/admin/ui/PageHeader", () => {
    const { Text, View } = require('react-native');
    return { PageHeader: ({ title }: any) => <View><Text>{title}</Text></View> };
});

jest.spyOn(Alert, 'alert');

describe("EditClass", () => {
    beforeEach(() => {
        jest.clearAllMocks();
        (classService.get as jest.Mock).mockResolvedValue({
            $id: "cl1",
            name: "Class A",
            semester: 2,
            course: { $id: "c1" },
            academicYear: { $id: "ay1" }
        });
    });

    it("loads data and updates successfully as Admin", async () => {
        (classService.update as jest.Mock).mockResolvedValue({});

        const { getByText, getByTestId } = render(<EditClass />);

        await waitFor(() => getByText("Edit Class"));
        expect(getByTestId("input-Name").props.value).toBe("Class A");

        fireEvent.changeText(getByTestId("input-Name"), "Class B");
        fireEvent.press(getByText("Save Changes"));

        await waitFor(() => {
            expect(classService.update).toHaveBeenCalledWith("cl1", expect.objectContaining({
                name: "Class B",
                semester: 2,
                course: "c1",
                academicYear: "ay1"
            }));
            expect(Alert.alert).toHaveBeenCalledWith("Success", "Class updated successfully");
        });
    });
});
