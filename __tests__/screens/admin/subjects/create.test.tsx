import CreateSubject from "@/app/(admin)/subjects/create";
...
describe("CreateSubject", () => {
    ...
    it("creates subject successfully", async () => {
        ...
        const { getByText, getByTestId } = render(<CreateSubject />);

        fireEvent.press(getByTestId("select-Course"));
        fireEvent.changeText(getByTestId("input-Subject Name"), "Data Structures");
        fireEvent.changeText(getByTestId("input-Subject Code"), "CS101");
        fireEvent.changeText(getByTestId("input-Semester"), "1");

        fireEvent.press(getByText("Create Subject"));

        await waitFor(() => {
            expect(subjectService.create).toHaveBeenCalledWith(expect.objectContaining({
            ...
            expect(Alert.alert).toHaveBeenCalledWith("Success", "Subject created successfully", expect.any(Array));
        });
    });
});
