  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={
        Platform.OS === "web" ? { flexGrow: 1 } : undefined
      }
        ...
        <View className="flex items-center justify-center px-6 pt-10 pb-32">
          ...
          >
            ...
            <Text className="text-lg md:text-xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed mb-10">
              The all-in-one platform for modern education. Streamline
              attendance, grading, and communication in one beautiful interface.
            </Text>
              ...
              <PremiumButton
                ...
                onPress={() => {
                  if (typeof window !== "undefined") {
                    // https://drive.google.com/drive/folders/1J5_UmEbEw7gHKOexNf1rkosBYxh9XXS_?usp=drive_link
        ...
        <View className="py-24 px-6 md:px-12 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800">
          <View className="max-w-6xl mx-auto">
            <View className="mb-20">
              <Text className="text-primary font-bold tracking-wider uppercase text-sm mb-2">
                Features
              </Text>
                ...
                <View className="absolute bottom-6 right-6 left-6 h-40 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 opacity-50 overflow-hidden">
                  <View className="grid grid-cols-4 gap-2 p-2">
                    {[1, 2, 3, 4].map((i) => (
                      <View
                        key={i}
                        className="h-8 bg-blue-500/20 rounded md:h-20"
                      />
                    ))}
                  </View>
                </View>
...
const PremiumButton = ({ label, icon, primary, onPress }: any) => {
  return (
    <Pressable
      onPress={onPress}
      className={`group px-6 py-3.5 rounded-full flex-row items-center gap-2 border transition-all active:scale-95 ${
        primary
    ...
    >
      <Text
        className={`font-bold text-base ${primary ? "text-white dark:text-slate-900" : "text-slate-700 dark:text-white"}`}
      >
        {label}
      </Text>
      {icon && (
        <Ionicons
          name={icon}
          size={18}
          className={`opacity-90 ${primary ? "text-white dark:text-black" : "text-slate-500"}`}
        />
      )}
    </Pressable>
  );
    ...
    <View className={boxClassName}>
      ...
      >
        ...
        <View className="p-8 relative z-10">
          ...
          <Text className="text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
            {desc}
          </Text>
        </View>
  ...
  >
    <View className="flex-row items-center justify-between mb-4">
      <Ionicons
        ...
        size={28}
        className={
          active ? "text-slate-400 dark:text-slate-600" : "text-slate-500"
        }
      />
      <Ionicons
        name="arrow-forward"
        size={20}
        className={active ? "text-white dark:text-slate-900" : "text-slate-500"}
      />
    </View>
    <Text
      className={`text-xl font-bold mb-1 ${active ? "text-white dark:text-slate-900" : "text-slate-900 dark:text-white"}`}
    >
      {title}
    </Text>
    <Text
      className={`font-medium ${active ? "text-slate-400 dark:text-slate-600" : "text-slate-500"}`}
    >
      {desc}
    </Text>
  </Pressable>
